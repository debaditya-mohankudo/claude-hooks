...
Environment variables (all optional, prefix CLAUDE_HOOKS_):
    CLAUDE_HOOKS_CLAUDE_DB_DIR   override ~/.claude DB directory (tool_hints, log)
    CLAUDE_HOOKS_ICLOUD_DB_DIR   override iCloud Databases directory (JSON stores only)
...
class _Config(BaseSettings):
    ...
    claude_db_dir: Path = Field(default=_CLAUDE_DIR_DEFAULT)
    memory_db: Path = Field(default=_CLAUDE_DIR_DEFAULT / "MEMORY.sqlite")
    ...
    def tool_hints_db(self) -> Path:
        # task:62ea0ea5 moved this off iCloud — a hot-path SQLite file under
        # iCloud sync is a known source of transient read locks / EDEADLK.
        return self.claude_db_dir / "tool_hints.sqlite"

    @computed_field
    @property
    def log_db(self) -> Path:
        # task:62ea0ea5 moved this off iCloud (row written on every log line).
        return self.claude_db_dir / "claude_hooks.sqlite"

    @computed_field
    @property
    def memory_scoring_json(self) -> Path:
        # Stays on iCloud: JSON, not a hot SQLite file, and cross-machine sync
        # of the scoring tunables is still wanted (task:62ea0ea5).
